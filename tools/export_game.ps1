...
$exports = @(
    ...
    @{ Preset = "android"; File = "FlappyRace.apk" }
)
...
foreach ($export in $exports) {
    ...
    $exportFlag = if ($export.Preset -eq "android") { "--export-debug" } else { "--export" }
    Start-Process -Wait -NoNewWindow $Godot -ArgumentList "--no-window", $exportFlag, $export.Preset, "$dir/$($export.File)"
