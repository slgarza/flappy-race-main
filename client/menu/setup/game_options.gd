...
onready var ItemMenu = $Panel/VBoxContainer/ItemMenu as MenuButton
...
func _ready() -> void:
	...
	for diff in CommonWorld.Difficulty.keys():
		...
		var value: int = CommonWorld.Difficulty[diff]
		difficulty_names[value] = text.capitalize()
	BotDifficultyInput.max_value = CommonWorld.Difficulty.size() - 1
	set_difficulty(CommonWorld.Difficulty.MEDIUM)
	MobileUI.adapt_game_options(self)
...
func set_enable_host_options(is_host: bool) -> void:
	...
	BotDifficultyInput.editable = is_host
	ItemsToggle.disabled = not is_host
	ItemMenu.disabled = not is_host
	MobileUI.adapt_game_options(self)
...
func set_game_options(game_options: Dictionary) -> void:
	...
	for i in game_options.item_ids_enabled.size():
		set_item_menu(i, game_options.item_ids_enabled[i])
...
func set_difficulty(value: int) -> void:
	BotDifficultyInput.value = value
	_update_difficulty_text(value)
	call_deferred("_update_difficulty_text", value)


func set_items(value: bool) -> void:
	ItemsToggle.pressed = value
	ItemMenu.visible = value


func set_item_menu(item_id: int, value: bool) -> void:
	var popup = ItemMenu.get_popup()
	popup.set_item_checked(item_id, value);
...
func _on_Difficulty_value_changed(value: float) -> void:
	...
	_update_difficulty_text(difficulty)
	call_deferred("_update_difficulty_text", difficulty)
	if emit_changes:
		Network.Client.game_options.difficulty = difficulty
		Network.Client.send_game_option_change("difficulty", difficulty)


func _on_ItemsToggle_toggled(toggle_value: bool) -> void:
	ItemMenu.visible = toggle_value
	if emit_changes:
		Network.Client.send_game_option_change("items", toggle_value)


func _update_difficulty_text(value: int) -> void:
	var text: String = difficulty_names.get(value, str(value))
	if BotDifficultyInput.has_method("get_line_edit"):
		BotDifficultyInput.get_line_edit().text = text
