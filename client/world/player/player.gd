...
var last_mobile_touch_flap_msec := -1000
...
func _input(event: InputEvent) -> void:
	...
	if is_mobile_touch_flap(event):
		...
		flap()
	elif is_duplicate_mobile_mouse_event(event):
		return
...
func is_mobile_touch_flap(event: InputEvent) -> bool:
	return (
		OS.get_name() == "Android"
...
func is_duplicate_mobile_mouse_event(event: InputEvent) -> bool:
	return (
		OS.get_name() == "Android"
		and event is InputEventMouseButton
		and OS.get_ticks_msec() - last_mobile_touch_flap_msec < 100
	)
